locals {
  istio_base = {
    helm_config = {
      name             = try(var.istio_ingress_extra_configs.istiobase_release_name, "istio-base")
      chart            = "base"
  ...
  istio_ingress = {
    helm_config = {
      name             = try(var.istio_ingress_extra_configs.istiod_ingress_release_name, "istio-ingress")
      chart            = "gateway"
      repository       = "https://istio-release.storage.googleapis.com/charts"
      version          = "1.21.1"
      namespace        = try(var.istio_ingress_extra_configs.namespace, "istio-ingress")
      create_namespace = try(var.istio_ingress_extra_configs.create_namespace, true)
      description      = "Istio helm Chart deployment configuration"
    }
  }
  ...
  default_helm_config = {
    name                       = try(var.istio_ingress_extra_configs.name, "istio-ingress")
    ...
    version                    = try(var.istio_ingress_extra_configs.version, "1.21.1")
    namespace                  = try(var.istio_ingress_extra_configs.namespace, "istio-system")
    create_namespace           = try(var.istio_ingress_extra_configs.create_namespace, true)
